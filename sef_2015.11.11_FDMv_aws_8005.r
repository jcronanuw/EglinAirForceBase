...
NFR <- c(54.38,457.39)#c(2000,8000)#Natural fire rotation in years for Eglin, Buffer, and Combined.
MFS <- c(103.65,5.23)#c(40,20)#Mean fire size in acres for Eglin, Buffer, and Combined.
...
Truncate.Number <- c(400,800,10,10,70)#Maximum number of fires and treatments in a given year
fire.cut <- 30#Number of acres under which fires will be removed (to speed up model run).
#Order is number of fires in Eglin, fires in buffer zone, and treatments (thinning, 
#herbicide, and prescribed fire) in Eglin.
Truncate.Treatment <- 100#NEW
...
#prescribed fire.
t.schedule <- read.table("sef_lut_schedule_treatment.csv", header=TRUE, 
                           sep=",", na.strings="NA", dec=".", strip.white=TRUE, stringsAsFactors = F)
t.schedule <- cleanup.import(t.schedule)
meanTreatments <- tschedule[z,]#c(1,1,1)# enter the mean number of treatments per year.
...
run.dat <- read.table("sef_lut_run_data.txt", header=TRUE, 
                     sep=",", na.strings="NA", dec=".", strip.white=TRUE)
run.dat <- cleanup.import(run.dat)
...
eglin.area <- length(b.map[!b.map %in% c(Buffer.Unit,NoData.Unit)])
...
if(length(loopB.new_stand) > 0)
{
  ...
  {
    #Updates f.map.
    f.map[s.map == loopB.new_stand[i]] <- ttxm[fblo[t.post$fuelbed == ufxTa[i]], loopB.treat_type[i]]
  ...
  stands <- (loopB.old_stand*-1)
  sareas <- loopB.new_area
  
  #Shelve fire history for stands that have been impacted by disturbance
  new_mfri_vec <- mapply(function(y) mfri.Matrix[Stand.List == y,], stands)
  nmv <- t(new_mfri_vec)
  ...
  Fuelbed.List <- c(Fuelbed.List,sfubes)
  Age.List <- c(Age.List,(sagess+1))
  T1E.List <- c(T1E.List,mapply(function(y) pda$thin[pda$pre == y],sfubes))
  T2E.List <- c(T2E.List,mapply(function(y) pda$herb[pda$pre == y],sfubes))
  D1E.List <- c(D1E.List,mapply(function(y) pda$sfire[pda$pre == y],sfubes))
  D2E.List <- c(D2E.List,mapply(function(y) pda$cfire[pda$pre == y],sfubes))
  Coord.List <- c(Coord.List,mapply(function(y) resample(l.map[s.map == y],1),loopB.new_stand))
...
#od.b <- od[order(od$ocod),]
#v.neef <- neef[match(od.b$osnd, osno)]
#s.map[l.map %in% ocod] <- v.neef
...
} else
{
  ...
  loopF.Area <- c(loopF.Area,mapply(function(x) length(s.map[s.map == x]),neef))
}
...
if(length(loopE.NewStand) > 0)
{
  ...
  standd <- loopE.ReplacedStand
  saread <- loopE.Area
  smud <- mapply(function(y) MU.List[Stand.List == y], loopE.ReplacedStand)

  #Shelve fire history for stands that have been impacted by disturbance
  new_mfri_vec <- mapply(function(y) mfri.Matrix[Stand.List == y,], standd)
  nmvd <- t(new_mfri_vec)
  ...
  Fuelbed.List <- c(Fuelbed.List,sfubed)
  Age.List <- c(Age.List,(sagesd+1))
  T1E.List <- c(T1E.List,mapply(function(y) pda$thin[pda$pre == y],sfubed))
  T2E.List <- c(T2E.List,mapply(function(y) pda$herb[pda$pre == y],sfubed))
  D1E.List <- c(D1E.List,mapply(function(y) pda$sfire[pda$pre == y],sfubed))
  D2E.List <- c(D2E.List,mapply(function(y) pda$cfire[pda$pre == y],sfubed))
  Coord.List <- c(Coord.List,mapply(function(y) resample(l.map[s.map == y],1),loopE.NewStand))
...
#Update f.map. (OLD A26 loop see script 
#sef_2015.11.11_codeTimer_Duplicate_a2.r for latest copy with loop)
...
write.table(s.map, file = paste("run_", run,"/sef_smap_",
                                dt,"_",tm,"_R",rows,"xC",cols,"_Y",a,".txt",sep = ""), 
            append = FALSE, quote = TRUE, sep = " ", eol = "\n", na = "NA", 
            dec = ".", row.names = FALSE,col.names = FALSE, qmethod = 
              c("escape", "double"))#

write.table(f.map, file = paste("run_", run,"/sef_fmap_",
                                dt,"_",tm,"_R",rows,"xC",cols,"_Y",a,".txt",sep = ""), 
            append = FALSE, quote = TRUE, sep = " ", eol = "\n", na = "NA", 
            dec = ".", row.names = FALSE,col.names = FALSE, qmethod = 
              c("escape", "double"))#
