...
find_neighbors_for_smap_cells <- function(smap_burning_cells, rows, cols)
{
  top <- function(x) { if ((x-1) %% rows == 1 || x-1 < 1) return (x) else return (x-1) }
  top_right <- function(x) { if ((x-1) %% rows == 1 || x-1 < 1 || x+rows > rows*cols) return (x) else return (x-1+rows) }
  right <- function(x) { if (x+rows > rows*cols) return (x) else return (x+rows) }
  bottom_right <- function(x) { if (x %% rows == 0 | x+rows > rows*cols) return (x) else return (x+1+rows) }
  bottom <- function(x) { if (x %% rows == 0) return (x) else return (x+1) }
  bottom_left <- function(x) { if (x %% rows == 0 | x-rows < 1) return (x) else return (x+1-rows) }
  left <- function(x) { if (x-rows < 1) return (x) else return (x-rows) }
  top_left <- function(x) { if ((x-1) %% rows == 1 | x-1 < 1 | x-rows < 1) return (x) else return (x-1-rows) }
  
  shift <- function(x, h, v)
    {
      x_row = if (x %% rows == 0) rows else (x %% rows)
      x_col = x %/% rows
      if (h < 1 - x_row | h > rows - x_row | v < 1 - x_col | v > cols - x_col) {
        return (x)
      } else {
        return (x + h*rows + v)
      }
  ...
  if (GPU_ENABLED) {
    ...
    neighbors <- matrix(smap_burning_cells, nrow=1, ncol=length(smap_burning_cells), byrow=TRUE)
    
    neighbors <- suppressWarnings(rbind(top(neighbors), top_right(neighbors), right(neighbors), bottom_right(neighbors),
                       bottom(neighbors), bottom_left(neighbors), left(neighbors), top_left(neighbors)))
  }
}
...
if(sum(meanTAP) <= 0)
  ...
  { #1.1.2-----------------------------------------------------------------------------
    ...
    { #2.0.0 ---------------------------------------------------------------------------
      ...
      t.code <- ifelse(length(which((meanTAA + meanUAA) < meanTAP)) == 0,4,min(which((meanTAA + meanUAA) < meanTAP)))
