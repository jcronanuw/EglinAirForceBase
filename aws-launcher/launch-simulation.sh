...
tail -n +2 $1 | while IFS=',' read count params; do
  aws ec2 run-instances --image-id ami-28b94148 --key-name EglinAirForceBase --user-data file://$SIM_ID/user_data_$line.ud --instance-type t2.large --count $count $SUBNET_ID
  ((line++))
done
